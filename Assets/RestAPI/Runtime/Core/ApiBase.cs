...
namespace RestAPI
{
    public abstract class ApiBase
    {
        ...
        private static KeyValuePair<string, string> AuthKeyPair = new(AuthKey, "");
        ...
        protected static void WebRequestMethod(string serviceUrl, string jsonData, string method,
            WebHelpers.CallbackGet callback)
        {
            if (string.IsNullOrEmpty(jsonData))
            {
                jsonData = "{}";
            ...
            else
            {
                ...
                WebHelpers.Instance.SendWithMethod<string>(serviceUrl, bytesContent, "application/json", callback,
                    AuthKeyPair, method);
            }
        }

        protected static void WebRequestForm(string serviceUrl, WWWForm formData, WebHelpers.CallbackGet callback,
            string method)
        {
            Debug.LogFormat("[][] requesting url {0}", serviceUrl);
        ...
        protected static void WebRequestGetImage(string serviceUrl, WebHelpers.CallbackGet callback)
        {
            ...
            Debug.LogFormat("[][] requesting url {0}", serviceUrl);
            WebHelpers.Instance.Get<Texture2D>(serviceUrl, callback, AuthKeyPair,false);
        }
